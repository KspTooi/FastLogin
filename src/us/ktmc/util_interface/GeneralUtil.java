...
public class GeneralUtil implements FastUtil{
	...
	public void PlayerRegister(Player PlayerEntity, String Passwd, String ConfirmPasswd) {
		...
		File PlayerConfig=getPlDataFile(PlayerEntity);
		...
		//判断ip是否已经达到限额
		if( ! (new PlayerIPCheck().MaxIPReg(PlayerEntity))){
			PlayerEntity.sendMessage("每个IP最多只能注册"+Var.ipmaxreg+"个账号,你已超出限额！");
			return ;
		}
		...
		try {
			...
			Var.V2.WriteAtKey(PlayerConfig, "password=", Passwd);
			Var.V2.WriteAtKey(PlayerConfig, "register=", "1");
			Var.V2.WriteAtKey(PlayerConfig, "login=", "1");
		...
		//将玩家传送到默认登陆位置
		TelePort_DefaultLoginLocation(PlayerEntity);
	...
	public boolean PlayerNameProcess(Player Player) {
		...
		//判断玩家名称长度
		if (Player.getName().length() < Var.MinNameLength) {
			Player.kickPlayer("玩家名过短 最小需要" + Var.MinNameLength + "个字符");
			return false;
		}
		//End
		...
		if(!Var.Regex.equalsIgnoreCase("*")){
			
			if(!Player.getName().matches(Var.Regex)){
